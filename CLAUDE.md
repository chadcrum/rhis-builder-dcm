...
Ansible role (`dcm_deploy`) that deploys DCM (Data Collection Manager) as podman quadlet containers managed by systemd on RHEL 9 / Fedora. Forked from `brianredbeard/rhis-builder-quay`.
...
**The role** (`roles/dcm_deploy/`) deploys 10 containers in six sequential phases defined in `tasks/main.yml`: prerequisites → generate_configs → deploy_quadlet_files → initialize_database → start_services → validate_deployment.
...
Optional providers (kubevirt, k8s-container, acm-cluster) are gated by `dcm_provider_*` boolean vars. Each provider's required vars are validated before templating — kubevirt/k8s-container only need a kubeconfig, but ACM needs five additional fields (see "Deploying Providers" above). The kubeconfig env var name differs per provider (`KUBERNETES_KUBECONFIG`, `SP_K8S_KUBECONFIG`, `KUBECONFIG`) — don't copy-paste between templates.
...
3. `roles/dcm_deploy/tasks/deploy_quadlet_files.yml` — add to core loop or as a conditional block with provider validation
...
6. `roles/dcm_deploy/tasks/initialize_database.yml` — add database name if the service needs one
